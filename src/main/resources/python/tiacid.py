...
DATAFORNUM = 10         # number of batch insert loop
...
connectionInfo = "connnect success, url=" + HOST + ":" + str(PORT) + ", username=" + USER + ", password=" + PASSWD + ", database=" + DB
print(connectionInfo)

cursor = connection.cursor()

print("start to init table data")
try:
    ...
    print("execute DDL failure, please rollback")
    connection.rollback()
print("create data successfully.")

n = 0

for i in range(BATCHSIZE * DATAFORNUM):
...
def runTransfer(connection):
    for i in range(TRANSFERFORNUM):
        ...
        try:
            ...
            print("execute transfer DML failure, please rollback")
            connection.rollback()
    connection.close()
...
for i in range(THREADNUM):
    ...
    thread.start()
    print("thread start successfully. thread id :=" + str(i))
...
print("start to check acid result.")
...
if int(totalBalance) != n * BALANCE:
    print("[ERROR] balance = " + str(totalBalance) + " is error, should be " + str(n * BALANCE))
else:
    print("[SUCCESS] balance = " + str(totalBalance) + ".")
if int(totalFlow) != 2 * TRANSFERFORNUM * THREADNUM:
    print("[ERROR] number of flow = " + str(totalFlow) + " is error, should be " + str(2 * TRANSFERFORNUM * THREADNUM))
else:
    print("[SUCCESS] number of flow = " + str(totalFlow) + ".")